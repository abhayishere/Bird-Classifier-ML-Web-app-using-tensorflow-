...
st.title("Image classifier using EfficientNetB0 model")
st.header("Bird species classifier")
st.text("Upload an image of a bird")
labels =['AFRICAN CROWNED CRANE', 'AFRICAN FIREFINCH', 'ALBATROSS',
       ...
       'BAR-TAILED GODWIT', 'BARN OWL', 'BARN SWALLOW', 'BARRED PUFFBIRD',
       'BAY-BREASTED WARBLER', 'BEARDED BARBET', 'BEARDED REEDLING',
       'BELTED KINGFISHER', 'BIRD OF PARADISE',
       'BLACK & YELLOW bROADBILL', 'BLACK FRANCOLIN', 'BLACK SKIMMER',
       ...
       'BLUE HERON', 'BOBOLINK', 'BORNEAN BRISTLEHEAD',
       'BORNEAN LEAFBIRD', 'BROWN NOODY', 'BROWN THRASHER',
       'BULWERS PHEASANT', 'CACTUS WREN', 'CALIFORNIA CONDOR',
       'CALIFORNIA GULL', 'CALIFORNIA QUAIL', 'CANARY',
       'CAPE MAY WARBLER', 'CAPUCHINBIRD', 'CARMINE BEE-EATER',
       'CASPIAN TERN', 'CASSOWARY', 'CEDAR WAXWING', 'CHARA DE COLLAR',
       'CHIPPING SPARROW', 'CHUKAR PARTRIDGE', 'CINNAMON TEAL',
       'CLARKS NUTCRACKER', 'COCK OF THE  ROCK', 'COCKATOO',
       ...
       'CRESTED NUTHATCH', 'CROW', 'CROWNED PIGEON', 'CUBAN TODY',
       'CURL CRESTED ARACURI', 'D-ARNAUDS BARBET', 'DARK EYED JUNCO',
       'DOUBLE BARRED FINCH', 'DOWNY WOODPECKER', 'EASTERN BLUEBIRD',
...
from img_classification import yoyo

file_uploaded = st.file_uploader("Choose a bird image..", type="jpg")
if file_uploaded is not None:
    img = Image.open(file_uploaded)
    st.image(img,caption="Uploaded bird image ", use_column_width=True)
    st.write("")
    st.write("Processing...")
    label = yoyo(img)
    st.write("The bird is : ",labels[label])